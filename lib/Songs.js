...
function getIDsFromPlaylist(playlistID, clbk) {
  dz.getIDsFromPlaylist(playlistID)
    .then(ids => {
      clbk(ids);
    })
    .catch(error => {
...
function getTrackFromID(id, clbk){
  dz.getTrack(id).then(track =>{
    clbk(track);
  })
}

async function getPlaylistIdFromURL(playlistUrl, clbk){
  dz.getPlaylistIdFromURL(playlistUrl).then(id=>{
    //console.log(id);
    clbk(id);
  })
}
...
exports.getTrackFromID = getTrackFromID;
exports.getIDsFromPlaylist = getIDsFromPlaylist;
