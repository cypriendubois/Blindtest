...
User.prototype.bindEvents = function() {
  ...
  this.socket.on("buzzerDecision", function(data) {
    ...
    var buzzerDecision = parseInt(data.buzzerDecision, 10);
  ...
  this.socket.on("answerTimeout", function(data) {
    var user = data.user;
    console.log("Resuming music and removing points if relevant for " + user + " on " + data.song +" song.");
    if (
      self.game.currentUser && 
      user == self.game.currentUser.username &&
      self.game.currentSong.title ==data.song
    ) {
